#!/usr/bin/env python3
"""
Complete solution to populate coordinator_monthly_summary table with all data
This handles the exact schema requirements and data type conversions properly
"""
...
def populate_coordinator_monthly_summary_complete():
    """Complete population of coordinator_monthly_summary with proper data handling"""
    try:
        ...
        # Complete solution that properly handles all the data requirements
        ...
        SELECT 
            ...
            -- Billing code mapping based on total minutes
        ...
        logger.info("Executing complete coordinator monthly summary population...")
        ...
        # Show sample data
        cursor.execute("""
            SELECT coordinator_id, month, year, total_minutes, patient_id, billing_code
        ...
        for row in sample_data:
            logger.info(f"  Coordinator {row['coordinator_id']}, {row['month']}/{row['year']}: {row['total_minutes']} minutes for patient {row['patient_id']} (Billing: {row['billing_code']})")
        
        conn.close()
        return True
        
    except Exception as e:
        logger.error(f"Error in complete coordinator monthly summary population: {e}")
        if 'conn' in locals():
            conn.close()
        return False

def verify_all_data():
    """Verify all data and table states"""
    try:
        conn = sqlite3.connect('production.db')
        cursor = conn.cursor()
...
if __name__ == "__main__":
    logger.info("Starting complete coordinator monthly summary population...")
    
    # First verify the data
    verify_all_data()
    
    # Then populate
    success = populate_coordinator_monthly_summary_complete()
    
    if success:
        logger.info("Complete coordinator monthly summary population completed successfully!")
        verify_all_data()
    else:
        logger.error("Complete coordinator monthly summary population failed!")
